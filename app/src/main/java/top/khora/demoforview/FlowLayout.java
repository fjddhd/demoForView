...
public class FlowLayout extends ViewGroup {
     ...
     * 参数的初始化除了借助类加载
     * 也可以在onMeasure中初始化
     * 但是不能在构造函数中初始化！因为一次构造后onMeasure等可能执行多次
     * */
    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mLine != null && mLine.getViewCount() > 0 && !mLines.contains(mLine)) {
            // 由于前面采用判断长度是否超过最大宽度来决定是否换行
            // ，则最后一行可能因为还没达到最大宽度，所以需要验证后加入集合中
            mLines.add(mLine);
        }
        ...
        // 设置布局的宽高，宽度直接采用父view传递过来的最大宽度，
        // 而不用考虑子view是否填满宽度，因为该布局的特性就是填满一行后，再换行
        ...
        public void LayoutView(int l, int t) {
            ...
            if (surplusWidth >= 0) {
                for (int i = 0; i < count; i++) {
                    ...
                    view.layout(left,top+topOffset,left+childWidth,
                            top + topOffset + childHeight);
